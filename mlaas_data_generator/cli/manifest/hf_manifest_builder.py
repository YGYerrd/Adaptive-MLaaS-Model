...
def _split_knobs_for_variant(task_key: str, knob_variant: int, resource_tier: ResourceTierSpec) -> tuple[str, float | None]:
    if resource_tier.rank == 0 or task_key in {"text_generation", "text2text_generation", "fill_mask"}:
        return "iid", None
    options: tuple[tuple[str, float | None], ...] = (("iid", None), ("dirichlet", 0.5), ("quantity_skew", 0.7))
    return options[int(knob_variant) % len(options)]
...
def _training_knobs_for_variant(
    *,
    ...
    split_strategy, distribution_param = _split_knobs_for_variant(task_key, knob_variant, resource_tier)
    if training_regime == "inference_only":
        return {
            "training_epochs": 0,
    ...
    lr_idx = (int(knob_variant) // len(batch_sizes)) % len(learning_rates)
    epoch_idx = (int(knob_variant) // (len(batch_sizes) * len(learning_rates))) % len(epochs)
    wd_idx = (int(knob_variant) // (len(batch_sizes) * len(learning_rates) * len(epochs))) % len(weight_decays)
    return {
...
def _row_from_generic_case(
    *,
    ...
    split_strategy, distribution_param = _split_knobs_for_variant(case["task_key"], knob_variant, resource_tier)
